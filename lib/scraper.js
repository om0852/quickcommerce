...
async function scrapePlatform(platform, searchUrls, pincode, category, proxyUrl, scrapedAt, retryCount = 0, currentApiKey = null) {
  ...
  try {
    ...
    const lastSnapshotQuery = { category, platform, pincode };
    if (platform === 'jiomart') {
      lastSnapshotQuery.isQuick = { $ne: false };
    }
    const lastSnapshot = await ProductSnapshot.findOne(lastSnapshotQuery).sort({ scrapedAt: -1 });
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const snapshot = new ProductSnapshot({
        ...
        isQuick: platform === 'jiomart' ? true : false,
...
/**
 * Scrape a specific custom category URL (bypassing the hardcoded CATEGORY_URLS map)
 * @param {Object} categoryObj - { name, url, officialCategory, officialSubCategory }
 * @param {Array} pincodes - List of pincodes to scrape
 */
export async function scrapeCustomCategory(categoryObj, pincodes) {
  const APIFY_PROXY_URL = process.env.APIFY_PROXY_URL || '';
  const { name: category, url, officialCategory, officialSubCategory } = categoryObj;

  if (!url) {
    throw new Error(`URL is required for custom category '${category}'`);
  }

  await dbConnect();

  // Fix the scrapedAt timestamp 
  const fixedScrapedAt = new Date();
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🕐 Starting CUSTOM scraping for: ${category}`);
  console.log(`🔗 URL: ${url}`);
  console.log(`📍 Pincodes: ${pincodes.join(', ')}`);
  console.log(`🕐 Timestamp: ${fixedScrapedAt.toISOString()}`);
  console.log(`${'='.repeat(80)}\n`);
  ...
  // Determine platform from URL (naive check)
  let platform = '';
  if (url.includes('blinkit.com')) platform = 'blinkit';
  else if (url.includes('zepto.com')) platform = 'zepto';
  else if (url.includes('jiomart.com')) platform = 'jiomart';
  else if (url.includes('dmart.in')) platform = 'dmart';
  else if (url.includes('swiggy.com')) platform = 'instamart';
  else throw new Error('Unknown platform in URL');

  const platformPromises = [];
  const searchUrls = [url];
  const categoryMetadata = { url, officialCategory, officialSubCategory };

  console.log(`\n🚀 Starting ${platform} scraping for ${pincodes.length} pincodes...`);

  if (platform === 'instamart') {
    // Instamart uses different logic currently
    platformPromises.push(
      (async () => {
        const instamartResults = [];
        for (const pincode of pincodes) {
          try {
            const result = await scrapeInstamart(url, pincode, category, fixedScrapedAt);
            instamartResults.push({ pincode, ...result });
          } catch (error) {
            console.error(`❌ Instamart scraping failed:`, error.message);
            instamartResults.push({ pincode, error: error.message });
          }
        }
        return { platform: 'instamart', status: 'fulfilled', results: instamartResults };
      })()
    ...
    // Use parallel scraper for others
    platformPromises.push(
      scrapePlatformParallel(platform, pincodes, category, searchUrls, APIFY_PROXY_URL, fixedScrapedAt, categoryMetadata)
        .then(results => ({ platform, status: 'fulfilled', results }))
        .catch(error => {
          console.error(`❌ ${platform} scraping failed:`, error.message);
          return { platform, status: 'rejected', reason: error };
        })
    );
  }
  ...
  platformResults.forEach((result) => {
    if (result.status === 'fulfilled' && result.value.status === 'fulfilled') {
      ...
      console.log(`\n✅ ${platform} scraping completed`);
    } else {
      console.error(`\n❌ Error scraping:`, result.reason || result.value?.reason);
    }
  });

  console.log(`\n✅ CUSTOM scraping completed for: ${category}\n`);
  return results;
}

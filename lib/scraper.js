...
async function scrapePlatform(platform, apiUrl, searchUrls, pincode, category, proxyUrl, retryCount = 0) {
  const MAX_RETRIES = 3;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (isMemoryError && retryCount < MAX_RETRIES) {
        ...
        return scrapePlatform(platform, apiUrl, searchUrls, pincode, category, proxyUrl, retryCount + 1);
      }
...
export async function scrapeCategory(category, pincodes) {
  ...
  for (const currentPincode of pincodes) {
    ...
    if (urls.zepto && urls.zepto.length > 0) {
      ...
      try {
        const result = await scrapePlatform('zepto', ZEPTO_API_URL, urls.zepto, currentPincode, category, APIFY_PROXY_URL);
    ...
    if (urls.blinkit && urls.blinkit.length > 0) {
      ...
      try {
        const result = await scrapePlatform('blinkit', BLINKIT_API_URL, urls.blinkit, currentPincode, category, APIFY_PROXY_URL);
    ...
    if (urls.jiomart && urls.jiomart.length > 0) {
      ...
      try {
        const result = await scrapePlatform('jiomart', JIOMART_API_URL, urls.jiomart, currentPincode, category, APIFY_PROXY_URL);

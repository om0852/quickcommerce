...
async function scrapePlatform(platform, apiUrl, searchUrls, pincode, category, proxyUrl, scrapedAt, retryCount = 0) {
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      let productId, productName, productImage, productWeight, quantity, deliveryTime, isAd, currentPrice, originalPrice, discountPercentage, productUrl;
      let dmartData = null;
      ...
      const snapshot = new ProductSnapshot({
        ...
        deliveryTime,
        isAd,
        rating: dmartData ? dmartData.rating : Number(item.rating || 0),
        currentPrice,
        originalPrice,
        discountPercentage,
        ranking,

...
export async function scrapeCategory(category, pincodes) {
  const APIFY_TOKEN_1 = process.env.APIFY_TOKEN_1;
  ...
  for (const currentPincode of pincodes) {
    ...
    if (urls.zepto && urls.zepto.length > 0) {
      ...
      promises.push(
        scrapePlatform('zepto', ZEPTO_API_URL, urls.zepto, currentPincode, category, null, fixedScrapedAt)
    ...
    if (urls.jiomart && urls.jiomart.length > 0) {
      ...
      promises.push(
        scrapePlatform('jiomart', JIOMART_API_URL, urls.jiomart, currentPincode, category, null, fixedScrapedAt)

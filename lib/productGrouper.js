...
export async function ungroupProduct(groupingId, platform, productId) {
    ...
    if (groupingId) {
        group = await ProductGrouping.findOne({ groupingId });
    } else {
        group = await ProductGrouping.findOne({
            products: { $elemMatch: { platform, productId } }
        });
    }

    if (!group) return false; // Nothing to ungroup
    ...
    if (group.products.length === originalLength) return false; // Product wasn't in list

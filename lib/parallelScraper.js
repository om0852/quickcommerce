...
async function scrapeSinglePincode(
    ...
    proxyUrl,
    scrapedAt,
    categoryMetadata = {}, // New parameter for metadata
    retryCount = 0
) {
    const MAX_RETRIES = 4;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (retryCount < MAX_RETRIES - 1) {
                ...
                return scrapeSinglePincode(platform, pincode, apiKey, keyIndex, searchUrls, category, proxyUrl, scrapedAt, categoryMetadata, retryCount + 1);
            }
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (platform === 'dmart') {
                ...
                // Append marketplace suffix for Flipkart
                if (platform.toLowerCase().includes('flipkart') && productUrl && !productUrl.includes('marketplace=HYPERLOCAL')) {
                    productUrl += (productUrl.includes('?') ? '&' : '?') + 'marketplace=HYPERLOCAL';
                }
            }
            ...
            const snapshot = new ProductSnapshot({
                ...
                // Metadata fields
                categoryUrl: categoryMetadata.url || searchUrls[0] || '', // Use metadata URL or fallback to first search URL
                officialCategory: categoryMetadata.officialCategory,
                officialSubCategory: categoryMetadata.officialSubCategory,
...
export async function scrapePlatformParallel(
    ...
    proxyUrl,
    scrapedAt,
    categoryMetadata = {} // New parameter
) {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        const promises = assignments.map(({ pincode, key, keyIndex }) =>
            scrapeSinglePincode(platform, pincode, key, keyIndex, searchUrls, category, proxyUrl, scrapedAt, categoryMetadata)
                .catch(error => {
